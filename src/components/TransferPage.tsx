...
const formatAccount = (raw: string) => {
  ...
  if (latin.toUpperCase().includes('IR')) {
...
interface Props {
  dests: Dest[]
  onDestsChange: (d: Dest[]) => void
  onSelect: (d: Dest) => void
}

export default function TransferPage({ dests, onDestsChange, onSelect }: Props) {
  ...
  const addDest = () => {
    ...
    if (!n || !a) return
    onDestsChange([{ name: n, account: formatAccount(a), badge: false, blue: false }, ...dests])
    setNewName('')
    setNewAccount('')
    setShowForm(false)
  }
  ...
  return (
    <div className="flex flex-col h-full bg-white my-10 relative">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-5 pb-[120px]">
        ...
        {dests.map((dest, i) => (
          <div key={i} className="flex items-center gap-[15px] py-3 cursor-pointer" onClick={() => onSelect(dest)}>
            <div className="relative">
              <div className={`w-[52px] h-[52px] rounded-full flex items-center justify-center flex-shrink-0 ${dest.blue
                ...
                {dest.blue ? (
                  ...
                  return bank ? (
                    <div className="w-full h-full bg-white rounded-full flex items-center justify-center">
                      ...
                      <div className={`absolute bottom-0 left-0 w-[22px] h-[22px] rounded-full overflow-hidden bg-white`}>
                        <IranianBank name={bank.iconKey ?? null} size={20} className='w-full h-full object-cover'/>
                      </div>
                    </div>
                  ) : (
                    <img src={avatar2} alt="" className="w-full h-full object-cover rounded-full" />
              ...
              {dest.badge && (
                <div className={`absolute bottom-0 left-0 w-[22px] h-[22px] rounded-full overflow-hidden ${dest.blue ? 'bg-primary' : 'bg-[#879FB1]'
                  }`}>
                  <img src="/Hb.webp" alt="" className="w-full h-full object-cover" />
                </div>
              )}
